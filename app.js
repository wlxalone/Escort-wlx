...
function getProfiles() {
  if (Array.isArray(window.EC_PROFILES)) return window.EC_PROFILES;
...
async function loadProfilesFromApi() {
  ...
  if (window.EC_DB) {
    try {
      const profiles = await window.EC_DB.loadProfiles();
      if (profiles.length > 0) {
        window.EC_PROFILES = profiles;
        return;
      }
    } catch (err) {
      console.warn('EliteConnect: ошибка Supabase, используем статические данные.', err);
    }
  }

  // Fallback: WordPress REST API
  const apiUrl = window.EC_CONFIG?.apiUrl;
  if (!apiUrl) return;
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      window.EC_PROFILES = data;
    }
  } catch (err) {
    console.warn('EliteConnect: не удалось загрузить профили из API, используем статические данные.', err);
  }
}
...
function createProfileCard(profile) {
  ...
  card.className = 'profile-card profile-card--editorial';
  ...
  if (ratesContainer) {
    ratesContainer.innerHTML = profile.rates
      .map(
        (rate) => `
            <div class="rate-box">
              <div class="rate-box__price">${localizePriceText(rate.price)}</div>
